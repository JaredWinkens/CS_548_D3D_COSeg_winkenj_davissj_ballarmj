...
def preprocess_with_labels(txt_file, labels_file, save_path):
    ...
    data = np.loadtxt(txt_file)  # [N x 7]
    labels = np.loadtxt(labels_file).astype(np.int32)  # [N]
    ...
    processed = np.concatenate([xyz, rgb, label_col], axis=1)  # [N x 7]
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess Semantic3D .txt + .labels into .npy")
    parser.add_argument('--data_path', type=str, required=True, help='Path to directory with .txt files')
