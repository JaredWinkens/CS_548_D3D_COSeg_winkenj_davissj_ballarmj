...
class OpenTrench3D_base(Dataset):
    def __init__(
        self,
        split="train",
        data_root="trainval",
        ...
        # Classes: { 0:'main_utility', 1:'other_utility', 2:'trench', 3:'inactive_utility', 4:'misc' }
        self.class_count = 5
        class_names = open(
            os.path.join(
                os.path.dirname(os.path.dirname(data_root)),
                "meta",
                "opentrench3d_classnames.txt",
            )
        ).readlines()
        self.class2type = {
        ...
        # misc never shows up, so we'll ignore that class
        self.fold_0 = ["main_utility", "trench"]
        self.fold_1 = ["inactive_utility", "other_utility"] 
...
class OpenTrench3D(OpenTrench3D_base):
    """
    The OpenTrench3D dataset class used for backbone pretraining.
    """
...
class OpenTrench3D_FS(OpenTrench3D_base):
    """
    The OpenTrench3D dataset class used for few-shot learning.
    """
...
class OpenTrench3D_FSForVIS(OpenTrench3D_FS):
    """
    The OpenTrench3D dataset class used for visulizaling the few-shot results.
    """
...
class OpenTrench3D_FS_TEST(Dataset):
    def __init__(
        self,
        split="val",
        data_root="trainval",
