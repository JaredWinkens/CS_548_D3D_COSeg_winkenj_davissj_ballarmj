            ...
            for file in glob.glob(os.path.join(self.data_root, "*.npy")):
                ...
                for class_id in classes:
                    if class_id < 0 or class_id >= self.class_count:
                        continue  # Skip unrecognized/invalid class labels
                    num_points = np.count_nonzero(labels == class_id)
                    threshold = max(int(data.shape[0] * min_ratio), min_pts)
                    if num_points > threshold:
                        class2scans[class_id].append(scan_name)
...
class Toronto3D(Toronto3D_base):
    """
...
class Toronto3D_FS(Toronto3D_base):
    """
...
class Toronto3D_FS_TEST(Dataset):
    def __init__(
        self,
        split="val",
        data_root="trainval",
